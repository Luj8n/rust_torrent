...
fn find_torrents(page: String) -> Result<(Vec<String>, Vec<String>), TorrentSearchError> {
  let torrent_res_re: Regex = Regex::new(TORRENT_RES_RE_STR).unwrap();
...
async fn find_magnet(url: &str) -> Result<String, TorrentSearchError> {
  let magnet_re: Regex = Regex::new(MAGNET_RE_STR).unwrap();
...
async fn find_peer_info(
  ...
  TorrentSearchError,
> {
  let seeds_re: Regex = Regex::new(SEEDS_RE_STR).unwrap();
  let leeds_re: Regex = Regex::new(LEECHES_RE_STR).unwrap();
  ...
  let leeches = match leeds_re.captures(&page) {
    Some(captures) => Ok(
      captures
        .get(1)
        .map_or("", |m| m.as_str())
